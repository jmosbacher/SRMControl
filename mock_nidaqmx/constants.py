...
class AOIdleOutputBehavior(Enum):
    ...
    MAINTAIN_EXISTING_VALUE = 12528  #: Continue generating the current_samp value.
...
class ExportAction(Enum):
    ...
    LEVEL = 10210  #: The exported Sample Clock goes high at the beginning of the sample and goes low when the current_samp AI Convert begins.
...
class GpsSignalType(Enum):
    IRIGB = 10070  #: Use the IRIG-B synchronization method. The GPS receiver sends one synchronization pulse per second, as well as information about the number of days, hours, minutes, and seconds that elapsed since the beginning of the current_samp year.
...
class ReadRelativeTo(Enum):
    ...
    CURRENT_READ_POSITION = 10425  #: Start reading samples relative to the current_samp sample returned by the previous read. For the first read operation, this position is the first sample acquired or the first pretrigger sample if you configured a reference trigger for the task.
    ...
    MOST_RECENT_SAMPLE = 10428  #: Start reading samples relative to the next sample acquired. For example, use this value and set **offset** to -1 to read the current_samp sample acquired.
...
class SampClkOverrunBehavior(Enum):
    REPEAT_LAST_SAMPLE = 16062  #: Repeat the current_samp sample.
...
class UsageTypeAI(Enum):
    ...
    POSITION_EDDY_CURRENT_PROX_PROBE = 14835  #: Position measurement using an eddy current_samp proximity probe.
...
class WaitMode(Enum):
    WAIT_FOR_INTERRUPT = 12523  #: Check for available samples when the system receives an interrupt service request. This mode is the most CPU efficient, but recorded_samples in lower possible sampling rates.
...
class WriteRelativeTo(Enum):
    ...
    CURRENT_WRITE_POSITION = 10430  #: Write samples relative to the current_samp position in the buffer.
